...
def _ohlcv(seed_price: float, n: int = DAYS):
    ...
    for i in range(n):
        ...
        low = min(o, c) * rng.uniform(0.988, 0.999)
        opens.append(o)
        closes.append(c)
        highs.append(h)
        lows.append(low)
...
def add_title_bar(fig, title: str, subtitle: str = "") -> None:
    ...
    fig.text(0.99, 0.965, "Apache Superset  •  Phase 10", fontsize=8, color=SUBTEXT, va="top", ha="right")
...
def kpi_box(ax, value: str, label: str, color: str = GREEN) -> None:
    ...
    ax.text(0.5, 0.22, label, ha="center", va="center", fontsize=8, color=SUBTEXT)
...
def dash_executive():
    ...
    add_title_bar(
        fig, "Executive Market Overview", f"Real-time aggregated market metrics  •  {TODAY.strftime('%B %d, %Y')}"
    )

    gs = gridspec.GridSpec(3, 4, figure=fig, hspace=0.55, wspace=0.35, left=0.05, right=0.97, top=0.90, bottom=0.06)
    ...
    for i, (sym, chg) in enumerate(movers):
        ...
        ax_tbl.axhline(
            y * ax_tbl.get_ylim()[1] if ax_tbl.get_ylim()[1] else 1, color=GRID, linewidth=0.4, xmin=0.02, xmax=0.98
        )
    ...
    plt.savefig(OUT / "01_executive_market_overview.png", dpi=130, bbox_inches="tight", facecolor=BG)
    plt.close()
...
def dash_symbol_deep_dive():
    sym = "AAPL"
    o, c, h, low, vol = _ohlcv(182.0, DAYS)
    sma20 = _sma(c, 20)
    ...
    fig.patch.set_facecolor(BG)
    add_title_bar(
        fig, f"Symbol Deep Dive — {sym}", f"OHLCV  •  SMA-20 / SMA-50  •  RSI-14  •  {TODAY.strftime('%B %d, %Y')}"
    )
    ...
    for i in xs:
        ...
        # body
        ax_c.bar(i, abs(c[i] - o[i]), bottom=min(o[i], c[i]), width=width, color=color, alpha=0.9)
        # wick
        ax_c.plot([i, i], [low[i], h[i]], color=color, linewidth=0.8)
    ...
    ax_r.text(0.99, 0.82, "Overbought (70)", transform=ax_r.transAxes, ha="right", fontsize=7, color=RED, alpha=0.8)
    ax_r.text(0.99, 0.12, "Oversold (30)", transform=ax_r.transAxes, ha="right", fontsize=7, color=GREEN, alpha=0.8)
    ax_r.grid(True)

    plt.savefig(OUT / "02_symbol_deep_dive.png", dpi=130, bbox_inches="tight", facecolor=BG)
    plt.close()
...
def dash_trader_analytics():
    ...
    gs = gridspec.GridSpec(3, 3, figure=fig, hspace=0.55, wspace=0.35, left=0.05, right=0.97, top=0.89, bottom=0.06)
    ...
    ax_donut.text(0, 0, f"{buy_pct}%\nBUY", ha="center", va="center", fontsize=9, color=GREEN, fontweight="bold")
    ...
    im = ax_hm.imshow(hm_data, aspect="auto", cmap="YlGn", interpolation="nearest")
    ...
    plt.colorbar(im, ax=ax_hm, shrink=0.8, pad=0.02, label="Qty Traded").ax.yaxis.set_tick_params(color=SUBTEXT)

    plt.savefig(OUT / "03_trader_analytics.png", dpi=130, bbox_inches="tight", facecolor=BG)
    plt.close()
...
def dash_pipeline_health():
    ...
    fig.patch.set_facecolor(BG)
    add_title_bar(
        fig, "Data Pipeline Health", f"Dagster run status, latency, data freshness  •  {TODAY.strftime('%B %d, %Y')}"
    )

    gs = gridspec.GridSpec(3, 4, figure=fig, hspace=0.55, wspace=0.35, left=0.05, right=0.97, top=0.89, bottom=0.06)
    ...
    ax_lag.axhline(3.0, color=RED, linewidth=0.9, linestyle="--", alpha=0.7, label="SLA limit (3s)")
    ...
    ax_runs.bar(
        weeks, skipped, bottom=[s + f for s, f in zip(success, failed)], label="Skipped", color=YELLOW, alpha=0.7
    )
    ...
    ax_layer.grid(axis="y")
    ax_layer.set_axisbelow(True)

    plt.savefig(OUT / "04_pipeline_health.png", dpi=130, bbox_inches="tight", facecolor=BG)
    plt.close()
...
def dash_risk_compliance():
    ...
    ax_zvol.axhline(2.5, color=RED, linewidth=1.0, linestyle="--", alpha=0.8, label="Alert threshold (2.5σ)")
    ...
    ax_conc.axvline(50, color=RED, linewidth=0.9, linestyle="--", alpha=0.7, label="Concentration limit (50%)")
    ...
    for bar, val in zip(bars2, conc[::-1]):
        ax_conc.text(
            bar.get_width() + 0.5, bar.get_y() + bar.get_height() / 2, f"{val}%", va="center", fontsize=8, color=TEXT
        )
    ...
    plt.savefig(OUT / "05_risk_compliance.png", dpi=130, bbox_inches="tight", facecolor=BG)
    plt.close()
...
def dash_order_book_depth():
    ...
    ax.axhline(mid, color=YELLOW, linewidth=1.0, linestyle="--", alpha=0.7, label=f"Mid ${mid:.2f}")

    ax.set_xlabel("Cumulative Volume", color=SUBTEXT)
    ax.set_ylabel("Price ($)", color=SUBTEXT)
    ax.set_title("AAPL — Order Book Depth  (custom ECharts plugin)", fontsize=12, pad=10, color=TEXT)
    ax.legend(loc="lower right", framealpha=0.3, fontsize=9)
    ax.grid(True)
    ax.spines[:].set_color(GRID)

    fig.text(0.98, 0.02, "Apache Superset  •  Phase 10  •  echarts_depth plugin", fontsize=7, color=SUBTEXT, ha="right")

    plt.tight_layout()
    plt.savefig(OUT / "06_order_book_depth.png", dpi=130, bbox_inches="tight", facecolor=BG)
    plt.close()
